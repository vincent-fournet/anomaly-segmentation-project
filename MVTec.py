...
class NormalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if "zipper" in self.img_files[idx] or "screw" in self.img_files[idx] or ("grid" in self.img_files[idx] and "grid_w" not in self.img_files[idx]):
...
class AbnormalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        """
        img = Image.open(self.img_files[idx])
        if "zipper" in self.img_files[idx] or "screw" in self.img_files[idx] or "grid_data" in self.img_files[idx]:
            img = np.expand_dims(np.array(img), axis=2)
            img = np.concatenate([img, img, img], axis=2)
            img = Image.fromarray(img.astype('uint8')).convert('RGB')
        if self.transform is not None:
...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        """
        img = Image.open(self.img_files[idx])
        if ("zipper" in self.img_files[idx] or "screw" in self.img_files[idx] or "grid_data" in self.img_files[idx] or "grid\\" in self.img_files[idx]):# and not ("other" in self.img_files[idx]):
            img = np.expand_dims(np.array(img), axis=2)
            img = np.concatenate([img, img, img], axis=2)
            img = Image.fromarray(img.astype('uint8')).convert('RGB')
        if self.transform is not None:
        ...
        # mask
        # h, w, _ = img.shape
        if img_name.split('\\')[-2] == "good" or img_name.split('\\')[-2] == "other":
