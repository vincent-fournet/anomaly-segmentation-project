...
class FeatCAE(nn.Module):
    ...
    def compute_energy(self, x, x_hat, save=False):
        if save != False:
            scores = (x - x_hat) ** 2
            print(save)
            np.save(save, scores.data.numpy())
        loss = torch.mean((x - x_hat) ** 2, dim=1)
        return loss

    def compute_energy_max(self, x, x_hat, save=False):
        loss = (x - x_hat) ** 2
        loss = 0.25 * (torch.mean(loss[:128], dim=1) + torch.mean(loss[128:384], dim=1) + torch.mean(loss[384:1408], dim=1) + torch.mean(loss[1408:3456], dim=1))
        return loss
